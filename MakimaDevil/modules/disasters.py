...
from MakimaDevil.modules.helper_funcs.extraction import extract_user
from MakimaDevil.modules.log_channel import gloggable
from telegram import ParseMode, TelegramError, Update
from telegram.ext import CallbackContext, CommandHandler, run_async
from telegram.utils.helpers import mention_html

ELEVATED_USERS_FILE = os.path.join(os.getcwd(), "MakimaDevil/elevated_users.json")
...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DEVILS:
        message.reply_text("This member is already a Devil")
        return ""

    if user_id in ANGELS:
        rt += "Requested {user} to promote a Angels to devils."
        data["supports"].remove(user_id)
        ANGELS.remove(user_id)

    if user_id in SENSEI:
        rt += "Requested {user} to promote a Sensei to devils."
        data["whitelists"].remove(user_id)
        SENSEI.remove(user_id)

    data["sudos"].append(user_id)
    DEVILS.append(user_id)
    ...
    update.effective_message.reply_text(
        rt
        + "\nSuccessfully set Disaster level of {} to devils!".format(
...
def addsupport(
    ...
    if user_id in DEVILS:
        rt += "Requested HA to demote this devils to Demon"
        data["sudos"].remove(user_id)
        DEVILS.remove(user_id)

    if user_id in ANGELS:
        message.reply_text("This user is already a Angels.")
        return ""

    if user_id in SENSEI:
        rt += "Requested HA to promote this Sensei to Demon"
        data["whitelists"].remove(user_id)
        SENSEI.remove(user_id)

    data["supports"].append(user_id)
    ANGELS.append(user_id)

    with open(ELEVATED_USERS_FILE, "w") as outfile:
        json.dump(data, outfile, indent=4)

    update.effective_message.reply_text(
        rt + f"\n{user_member.first_name} was added as a Angels!"
    )
...
def addwhitelist(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DEVILS:
        rt += "This member is a Devil, Demoting to Wolf."
        data["sudos"].remove(user_id)
        DEVILS.remove(user_id)

    if user_id in ANGELS:
        rt += "This user is already a Angels, Demoting to Wolf."
        data["supports"].remove(user_id)
        ANGELS.remove(user_id)

    if user_id in SENSEI:
        message.reply_text("This user is already a Sensei.")
        return ""

    data["whitelists"].append(user_id)
    SENSEI.append(user_id)

    with open(ELEVATED_USERS_FILE, "w") as outfile:
        json.dump(data, outfile, indent=4)

    update.effective_message.reply_text(
        rt + f"\nSuccessfully promoted {user_member.first_name} to a Sensei!"
    )
...
def addtiger(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DEVILS:
        rt += "This member is a Devil, Demoting to Tiger."
        data["sudos"].remove(user_id)
        DEVILS.remove(user_id)

    if user_id in ANGELS:
        rt += "This user is already a Angels, Demoting to Tiger."
        data["supports"].remove(user_id)
        ANGELS.remove(user_id)

    if user_id in SENSEI:
        rt += "This user is already a Sensei, Demoting to Tiger."
        data["whitelists"].remove(user_id)
        SENSEI.remove(user_id)
...
def removewhitelist(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in SENSEI:
        ...
        message.reply_text("This user is not a Sensei!")
        return ""
...
def whitelistlist(update: Update, context: CallbackContext):
    reply = "<b>Known Senseis 🐺:</b>\n"
    m = update.effective_message.reply_text(
        "<code>Gathering intel..</code>", parse_mode=ParseMode.HTML
    )
    bot = context.bot
    for each_user in SENSEI:
...
def supportlist(update: Update, context: CallbackContext):
    ...
    reply = "<b>Known Angelss 👹:</b>\n"
    for each_user in ANGELS:
        user_id = int(each_user)
        try:
            user = bot.get_chat(user_id)
            reply += f"• {mention_html(user_id, html.escape(user.first_name))}\n"
...
# *Global Blue Text*
#  ❍ /gignoreblue*:* <word>*:* Globally ignorea bluetext cleaning of saved word across MakimaDevil.
...
SUDO_HANDLER = CommandHandler(("addsudo", "adddevils"), addsudo)
SUPPORT_HANDLER = CommandHandler(("addsupport", "adddemon"), addsupport)
TIGER_HANDLER = CommandHandler(("addtiger"), addtiger)
WHITELIST_HANDLER = CommandHandler(("addwhitelist", "addwolf"), addwhitelist)
UNSUDO_HANDLER = CommandHandler(("removesudo", "removedevils"), removesudo)
